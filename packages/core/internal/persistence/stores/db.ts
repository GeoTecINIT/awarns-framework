import { CouchBase, Query as Q, QueryLogicalOperator, QueryMeta, QueryWhereItem as QWI } from '@triniwiz/nativescript-couchbase';
...
export class NotUniqueErr extends Error {
  constructor(ids: string | Array<string>) {
    super(typeof ids === 'string' ? `Document with id (${ids}) already exists (perhaps with a different doc type)` : `Documents with ids (${JSON.stringify(ids)}) already exist (perhaps with a different doc type). No change has been committed`);
  }
}
