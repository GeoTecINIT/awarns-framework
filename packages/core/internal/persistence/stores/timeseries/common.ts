...
export abstract class AbstractTimeSeriesStore<T extends TimeSeriesEntity> implements LocalTimeSeriesStore<T> {
  ...
  protected constructor(docType: string, private serialize: (entity: T) => TimeSeriesDoc, deserialize: (doc: TimeSeriesDoc) => T, private keepId = false) {
    ...
    this.store = new EMAIStore<T & DBEntityProps>(docType, serializer, deserializer);
  }
